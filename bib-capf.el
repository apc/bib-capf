...
(declare-function citar--bibliographies "ext:citar")
(declare-function citar-cache--entries "ext:citar")
...
(defun bib-capf--bibtex-get-buffer-field-values (field)
  "Parse buffer to collect values for FIELD."
  (let (raw-vals)
    (bibtex-map-entries
...
(defun bib-capf--bibtex-get-field-values (field &optional global)
  "Get all values for FIELD from current buffer.

If optional argument GLOBAL is non-nil, collect values in all
files in `bibtex-files’."
  (if (not global)
